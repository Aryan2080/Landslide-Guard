"""Detection V2 - CPU orchestrator (Windows, 12 threads).

Same experimental design as scripts/stage2_v2_kaggle_train.py, but sized for
a CPU-only box (~2 min/epoch with base_features=16, batch=16).

Fair-comparison protocol per Phase 5-8:
    - Phase 5 (architectures): 3 arms at loss=bce_dice, same protocol A.
    - Phase 6 (losses):        3 arms on arch winner, same protocol B.
    - Phase 11: model selection by validation Dice (tiebreak IoU, recall).
    - Phase 12: threshold sweep on validation.
    - Phase 13: LOCK + one-shot test evaluation.
    - Phases 14-22: test gallery, error categories, GeoJSON polygon,
      V1-vs-V2 comparison, readiness report.

All outputs go under WORK/{outputs/detection/v2_{reports,training,predictions},
models_v2, checkpoints_v2}. V1 artifacts are untouched.
"""
...
# ---- paths (overrideable via env vars) ------------------------------------
REPO_DIR = Path(os.environ.get("LANDSLIDE_REPO_DIR",
                               r"D:\LANDSLIDE\LandslideGuard"))
DATA_ROOT = Path(os.environ.get("LANDSLIDE_DATA_ROOT",
                                str(REPO_DIR / "data" / "raw" / "landslide4sense")))
WORK = Path(os.environ.get("LANDSLIDE_WORK_DIR", str(REPO_DIR)))
...
V2_TAG = "v2_cpu_" + datetime.now().strftime("%Y%m%d_%H%M%S")


# ---- CPU-adapted protocol -------------------------------------------------
# Fewer epochs than GPU orchestrator, same-across-arms fair protocol.
ARCH_PROTOCOL = TrainProtocol(
    seed=42,
    epochs=int(os.environ.get("V2_ARCH_EPOCHS", "10")),
    early_stop_patience=int(os.environ.get("V2_ARCH_PATIENCE", "6")),
...
LOSS_PROTOCOL = TrainProtocol(
    seed=42,
    epochs=int(os.environ.get("V2_LOSS_EPOCHS", "15")),
    early_stop_patience=int(os.environ.get("V2_LOSS_PATIENCE", "8")),
    lr=float(os.environ.get("V2_LR", "1e-3")),
    weight_decay=float(os.environ.get("V2_WD", "1e-4")),
    grad_clip=1.0,
    use_amp=False,
...
BATCH_SIZE = int(os.environ.get("V2_BATCH", "16"))
NUM_WORKERS = int(os.environ.get("V2_WORKERS", "0"))  # 0 on Windows to avoid spawn overhead
BASE_FEATURES = int(os.environ.get("V2_BASE_FEATURES", "16"))
CPU_THREADS = int(os.environ.get("V2_CPU_THREADS", "12"))
torch.set_num_threads(CPU_THREADS)

# Architecture experiments (Phase 5) - all at bce_dice with ARCH_PROTOCOL
ARCH_MATRIX = [
    ("baseline_bn",  {"base_features": BASE_FEATURES, "norm": "batchnorm", "residual": False}),
    ("baseline_gn",  {"base_features": BASE_FEATURES, "norm": "groupnorm", "residual": False}),
    ("residual_bn",  {"base_features": BASE_FEATURES, "norm": "batchnorm", "residual": True}),
]

# Loss experiments (Phase 6) - all with LOSS_PROTOCOL
LOSS_MATRIX = [
...
# ---- dataset resolution (Stage-1 nested layout) --------------------------
def resolve(split: str) -> tuple[Path, Path]:
    """Handles both flat and archive-nested layouts."""
    candidates = [
...
def make_loaders(stats):
    ds_train = Landslide4SenseDataset(*resolve("TrainData"), stats=stats,
                                      split="train", augment_seed=ARCH_PROTOCOL.seed)
...
def write_env_report(dst: Path, device: torch.device) -> Path:
    import platform
    lines = [
        f"# Detection V2 (CPU) - Phase 2 - Environment report",
        ...
        f"seed             : {ARCH_PROTOCOL.seed}",
        f"REPO_DIR         : {REPO_DIR}",
        f"DATA_ROOT        : {DATA_ROOT}",
        f"WORK             : {WORK}",
        f"BATCH_SIZE       : {BATCH_SIZE}",
        f"BASE_FEATURES    : {BASE_FEATURES}",
        f"ARCH_PROTOCOL    : {ARCH_PROTOCOL.__dict__}",
        f"LOSS_PROTOCOL    : {LOSS_PROTOCOL.__dict__}",
    ]
...
def train_arm(arm_id: str,
    ...
    tracker.append(ExperimentRow(
        experiment_id=f"{V2_TAG}_{arm_id}",
        model=f"unet_v2_{arch_kwargs.get('norm','bn')}"
              + ("_res" if arch_kwargs.get('residual') else ""),
        in_channels=14, out_channels=1,
        base_features=int(arch_kwargs.get("base_features", BASE_FEATURES)),
        ...
        notes=f"arch={arch_id} loss={loss_id} phase={'arch' if protocol is ARCH_PROTOCOL else 'loss'}",
    ))

    # per-arm training curves (CSV + PNG)
...
def load_model_from_checkpoint(ckpt_path: Path, device: torch.device) -> torch.nn.Module:
    ...
    model = UNetV2(
        ...
        base_features=int(arch.get("base_features", BASE_FEATURES)),
...
def error_categories_v2(model, loader, device, threshold, out_dir: Path) -> dict:
    ...
    cats = {
        "n_total": total, "n_no_gt": len(no_gt), "n_has_gt": len(has_gt),
...
def main() -> None:
    ...
    # Phase 3: normalization stats (frozen) + data validation
    ...
    # train-only positive-pixel ratio -> pos_weight
    ...
    # ==== Phase 5 - Architecture experiments (bce_dice fixed, ARCH_PROTOCOL)
    log("=" * 70); log(f"Phase 5 - ARCHITECTURE EXPERIMENTS "
                       f"(loss=bce_dice, epochs={ARCH_PROTOCOL.epochs})"); log("=" * 70)
    arch_rows = []
    ...
    # ==== Phase 6 - Loss experiments on arch winner (LOSS_PROTOCOL)
    ...
    log("=" * 70); log(f"Phase 6 - LOSS EXPERIMENTS "
                       f"(arch={winning_arch_id}, epochs={LOSS_PROTOCOL.epochs})"); log("=" * 70)
    loss_rows = []
    ...
    (v2_reports / "threshold_selection.md").write_text(
        f"# Detection V2 (CPU) - Phase 12 - Threshold Selection\n\n"
    ...
    # ==== Phase 20 - final config + LOCKED marker
    final_cfg = {
        "run_tag": V2_TAG,
        ...
        "optimizer": "adamw", "learning_rate": LOSS_PROTOCOL.lr,
        "weight_decay": LOSS_PROTOCOL.weight_decay, "scheduler": "cosine",
        "batch_size": BATCH_SIZE, "seed": ARCH_PROTOCOL.seed,
        "checkpoint": str(best_ckpt),
        "threshold": LOCKED_THRESHOLD,
        "training_positive_pixel_ratio": p_pos,
        "selected_by": "val_dice (tiebreak val_iou, val_recall)",
        "cpu_run": True,
        "arch_protocol_epochs": ARCH_PROTOCOL.epochs,
        "loss_protocol_epochs": LOSS_PROTOCOL.epochs,
    }
    import yaml
    (WORK / "configs" / "detection_v2_final.yaml").parent.mkdir(exist_ok=True)
    (WORK / "configs" / "detection_v2_final.yaml").write_text(
        yaml.safe_dump(final_cfg, sort_keys=False))
    log("wrote configs/detection_v2_final.yaml + LOCKED marker")
    write_lock(best_ckpt.parent, reason=f"v2 lock {V2_TAG}")

    export_dir = WORK / "models" / "detection"; export_dir.mkdir(parents=True, exist_ok=True)
    ...
    (export_dir / "detection_v2_metadata.json").write_text(
    ...
    (v2_reports / "v1_vs_v2_comparison.md").write_text(
        ...
        f"Note: V2 CPU run used the same base_features (16) as V1 for tractable\n"
        f"wall-clock. The V2 gain (if any) comes from either the loss choice or\n"
        f"the fairer training protocol, not from a larger model.\n"
    )

    # ==== Phase 22 - final report + readiness
    report = {
        "run_tag": V2_TAG,
        "device": device_summary(device),
        "arch_protocol": ARCH_PROTOCOL.__dict__,
        "loss_protocol": LOSS_PROTOCOL.__dict__,
    ...
    log("STAGE 2 V2 CPU RUN COMPLETE")
